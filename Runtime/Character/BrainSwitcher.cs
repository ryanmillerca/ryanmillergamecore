namespace RyanMillerGameCore.Character {
	...
	public class BrainSwitcher : MonoBehaviour {
		...
		private ICharacterReferenceProvider m_characterReferenceProvider;

		private void Awake() {
			m_characterReferenceProvider = GetComponentInChildren<ICharacterReferenceProvider>();
		}
		...
		public void SwitchToBrain(int index) {
			...
			for (int i = 0; i < brains.Length; i++) {
				if (i == index) {
					...
					m_characterReferenceProvider.CharacterBrain = brains[i];
				}
				else {
