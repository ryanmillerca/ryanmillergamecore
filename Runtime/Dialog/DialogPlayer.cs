namespace RyanMillerGameCore.Dialog
{
    ...
    public class DialogPlayer : MonoBehaviour
    {
        ...
        public void OnEnable()
        {
            ...
            _actionMap = inputActionAsset != null ? inputActionAsset.FindActionMap("UI", true) : null;
            if (_actionMap == null)
            {
                Log("UI action map not found on InputActionAsset.", true);
                return;
            }

            _submitAction = _actionMap.FindAction("Submit", true);
            _submitAction.performed += OnDialogButtonPressed;
            _submitAction.Enable();

            _cancelAction = _actionMap.FindAction("Cancel", true);
